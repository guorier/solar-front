  ...
  return (
    <AriaListBoxItem {...props} textValue={textValue}>
      {composeRenderProps(props.children, (children) =>
        typeof children === 'string' ? <p slot="label">{children}</p> : children,
      )}
    </AriaListBoxItem>
  );
}
...
export function DropdownItem(props: ListBoxItemProps) {
  ...
  return (
    <AriaListBoxItem {...props} textValue={textValue} className="dropdown-item">
      {composeRenderProps(props.children, (children, { isSelected, selectionMode }) => (
        <>
          {selectionMode === 'multiple' && (
            <Checkbox isSelected={isSelected} isReadOnly aria-hidden="true" />
          )}

          {typeof children === 'string' ? <span slot="label">{children}</span> : children}
        </>
      ))}
    </AriaListBoxItem>
  );
